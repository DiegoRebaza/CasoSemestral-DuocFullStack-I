...
@RestController
@RequestMapping("/api/posventa/v2")
public class PosventaControllerV2 {

    private static final Logger log = LoggerFactory.getLogger(PosventaControllerV2.class);

    private final PosventaService posventaService;
    private final PosventaModelAssembler assembler;

    public PosventaControllerV2(PosventaService posventaService, PosventaModelAssembler assembler) {
        this.posventaService = posventaService;
        this.assembler = assembler;
    }

    // POST /api/posventa/v2
    @PostMapping
    public ResponseEntity<PosventaDTO> crearPosventa(@Valid @RequestBody PosventaDTO posventaDTO) {
        log.info("POST /api/posventa/v2 - Recibida solicitud para registrar posventa, clienteId={}, compraId={}",
    ...
    public ResponseEntity<CollectionModel<EntityModel<PosventaDTO>>> listarPosventas() {
        log.info("GET /api/posventa/v2 - Recibida solicitud para listar todas las posventas");
        ...
        CollectionModel<EntityModel<PosventaDTO>> resultado = CollectionModel.of(posventasModel,
                linkTo(methodOn(PosventaControllerV2.class).listarPosventas()).withSelfRel());

        log.info("Retornando lista con {} posventas", lista.size());
        return ResponseEntity.ok(resultado);
    }

    // GET /api/posventa/v2/{id}
    @GetMapping("/{id}")
    public ResponseEntity<EntityModel<PosventaDTO>> obtenerPosventa(@PathVariable Long id) {
        log.info("GET /api/posventa/v2/{} - Buscando posventa por ID", id);
    ...
    // GET /api/posventa/v2/{id}/exists
    @GetMapping("/{id}/exists")
    public ResponseEntity<Boolean> existePosventa(@PathVariable Long id) {
        log.info("GET /api/posventa/v2/{}/exists - Comprobando existencia de posventa", id);
        return ResponseEntity.ok(posventaService.existePorId(id));
    }

    // PUT /api/posventa/v2/{id}
    @PutMapping("/{id}")
    public ResponseEntity<EntityModel<PosventaDTO>> actualizarPosventa(
            @PathVariable Long id, @Valid @RequestBody PosventaDTO posventaDTO) {
        log.info("PUT /api/posventa/v2/{} - Recibida solicitud de actualización de posventa", id);
    ...
    // DELETE /api/posventa/v2/{id}
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> eliminarPosventa(@PathVariable Long id) {
        log.info("DELETE /api/posventa/v2/{} - Recibida solicitud para eliminar posventa", id);
